...
export const DataStorageDataContext =
    React.createContext<IDataStorageDataContext>({
        levelFilter: new Set(),
...
export const DataStorageSetterContext =
    React.createContext<IDataStorageSetterContext>({
        setLevelFilter: () => {},
        setExcludedClasses: () => {},
        setSearches: () => {},
        setFavourites: () => {},
...
export const useDataStorageDataContext = () =>
    React.useContext(DataStorageDataContext);
export const useDataStorageSetterContext = () =>
    React.useContext(DataStorageSetterContext);

const STORAGE_KEY = 'LOGS_DATA_STORAGE';
const LAST_SAVE_KEY = 'LAST_SAVE';
const DATA_STORE_TIME = 1000 * 60 * 60 * 24 * 7; // 1 week
export const DEFAULT_LEVEL_FILTER = [
    LogLevel.WARNING,
    LogLevel.ERROR,
    LogLevel.UNKNOWN,
];
...
export const DataStorageProvider: React.FC<React.PropsWithChildren> = ({
    children,
}) => {
    ...
    const [levelFilter, setLevelFilter] = React.useState(
        new Set(DEFAULT_LEVEL_FILTER)
    );
    const [excludedClasses, setExcludedClasses] = React.useState(
        new Set<string>()
    );
    const [searches, setSearches] =
        React.useState<ISearchFilter[]>(getDefaultSearches);
    ...
    React.useEffect(() => {
        ...
        const stored = storedData.current.find(
        ...
        if (stored) {
            ...
            setFavourites(
                new Set(stored.favourites[currentSession?.name || ''] ?? [])
            );
    ...
    React.useEffect(() => {
        if (
            !dataHash ||
            dataHash !== loadedDataHash.current ||
            currentSession?.name !== loadedSessionName.current
        ) {
            return;
        }
        // Load data from storage to not override data saved by other tabs
        loadNewestData();
        const storedIndex = storedData.current.findIndex(
            (data) => data.hash === dataHash
        );
    ...
    const dataValue = React.useMemo(
        ...
        [levelFilter, excludedClasses, searches, favourites]
    );
