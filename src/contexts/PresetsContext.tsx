...
const DEFAULT_PRESETS: IPreset[] = [
    ...
    },
    {
        ...
        data: {
            ...
            includedClasses: ['VideoLayoutItemHandler'],
        },
    },
];

export const PresetsProvider: React.FC<React.PropsWithChildren> = ({
    children,
}) => {
    const [presets, setPresets] = React.useState<IPreset[]>(DEFAULT_PRESETS);

    const updatePresets = React.useCallback(
        (newPresets: IPreset[]) => {
    ...
    const update = React.useCallback(
        (oldPreset: IPreset, newPreset: IPreset) => {
            const index = presets.findIndex((p) =>
                isMatchPreset(p, oldPreset.name, oldPreset.category)
            );
            if (index === -1) {
                add(newPreset);
                return;
    ...
    const importPresets = React.useCallback(
        (newPresets: IPreset[]) => {
            const mergedPresets = newPresets.reduce((acc, preset) => {
                const index = acc.findIndex((p) =>
                    isMatchPreset(p, preset.name, preset.category)
                );
                if (index === -1) {
                    return [...acc, preset];
                }
    ...
    const deletePreset = React.useCallback(
        (preset: IPreset) => {
            const newPresets = presets.filter(
                (p) => !isMatchPreset(p, preset.name, preset.category)
            );
            updatePresets(newPresets);
        },
        [presets, updatePresets]
